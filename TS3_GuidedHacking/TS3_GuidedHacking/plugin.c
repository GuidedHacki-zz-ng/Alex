/*
* A plugin designed for Alex' shitty trolls.
*
* Plugin Base:
...
const char* ts3plugin_name() {
	...
	if (!result) {
		const wchar_t* name = L"GH-Plug";
		if (wcharToUtf8(name, &result) == -1) {  /* Convert name into UTF-8 encoded result */
			result = "GH-Plug";  /* Conversion failed, fallback here */
		}
	}
	return result;
#else
...
/* Plugin author */
/*Edits from original:
Added a convert from UTF-16 to *-8 */
const char* ts3plugin_author() {
#ifdef _WIN32
	static char* result = NULL; 
	if (!result) {
		const wchar_t* name = L"Alex";
		if (wcharToUtf8(name, &result) == -1) {  /* Convert to UTF-8 */
			result = "Alex";
		}
	}
	return result;
...
const char* ts3plugin_description() {
	...
	return "This plugin allows GH-Staff to manipulate TS3 in an easier way.";
}
...
void ts3plugin_initMenus(struct PluginMenuItem*** menuItems, char** menuIcon) {
	...
	*/
	...
	CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_CLIENT, MENU_ID_CLIENT_1, "Poke user", "1.png");
...
void ts3plugin_onMenuItemEvent(uint64 serverConnectionHandlerID, enum PluginMenuType type, int menuItemID, uint64 selectedItemID) {
	...
	ts3Functions.getClientDisplayName(serverConnectionHandlerID, myClientID, myName, sizeof(myName));*/
	...
	case PLUGIN_MENU_TYPE_CLIENT:
		...
		case MENU_ID_CLIENT_1:
			...
			{
				ts3Functions.requestClientPoke(serverConnectionHandlerID, selectedItemID, "Rekt by Alex' Hax", "AlexHax");
				SleepEx(100, 1);
			}
			break;
